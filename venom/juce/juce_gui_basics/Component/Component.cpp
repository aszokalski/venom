...
void init_Component(py::module &m) {
    py::class_<juce::Component, PyComponent>(m, "Component", py::dynamic_attr())
            .def(py::init<>())
            .def("paint", [](juce::Component &self, juce::Graphics &g) { self.paint(g); })
            .def("resized", [](juce::Component &self) { self.resized(); })
            .def("getName", [](juce::Component &self) { return self.getName(); })
