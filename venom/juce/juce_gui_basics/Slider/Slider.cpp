...
//    py::class_<juce::Component>(m, "Component").def(py::init<>());
    auto slider = py::class_<juce::Slider, juce::Component>(m, "Slider", py::dynamic_attr())
            .def(py::init<>([]() {
                return createOnMessageThread<juce::Slider>([] {
                    return new juce::Slider();
                });
            }))
            .def(py::init([](const juce::String &componentName) { return new juce::Slider(componentName); }))
            ...
            .def("setSliderStyle",
                 [](juce::Slider &self, juce::Slider::SliderStyle newStyle) { self.setSliderStyle(newStyle); })
            .def("getSliderStyle", [](juce::Slider &self) { return self.getSliderStyle(); })
            ...
            .def("setVisible", [](juce::Slider &self, bool shouldBeVisible) { self.setVisible(shouldBeVisible); })
            .def("setBounds", [](juce::Slider &self, int x, int y, int w, int h) { self.setBounds(x, y, w, h); })
            .def("setSize", [](juce::Slider &self, int newWidth, int newHeight) { self.setSize(newWidth, newHeight); });
