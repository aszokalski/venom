...
from venom.builder.tools import project, template
...
@click.option(
    "--targets",
    default=[],
    help="Targets to build [Standalone, VST3, AU]",
    multiple=True,
)
...
@click.command(
    help="Allows faster building (for dev testing) on macOS. It only replaces the juce bindings."
)
@click.option("--source-path", default=".", help="Path to the source directory")
@click.option(
    "--juce-bindings-path",
    default="../venv/lib/python3.11/site-packages/juce.cpython-311-darwin.so",
    help="Path to the source directory",
)
@click.option(
    "--mac-bundle-path",
    default="./dist/Standalone/VenomPlugin1.app",
    help="Path to a macOS bundle",
)
def test(source_path, juce_bindings_path, mac_bundle_path):
    ...
    with open(os.path.join(source_path, "venom.yaml")) as file:
        ...
        shutil.copy(
            os.path.join(source_path, config.entrypoint),
            os.path.join(
                mac_bundle_path,
                "Contents",
                "Resources",
        ...
        for file in tqdm(os.listdir(source_path), desc="Copying python files"):
            if file.endswith(".py") and file != config.entrypoint:
                shutil.copy(
                    ...
                    os.path.join(
                        mac_bundle_path, "Contents", "Resources", "site-packages", file
                    ),
                )
