...
void init_AudioProcessor(py::module &m) {
    py::class_<juce::AudioProcessor, std::shared_ptr<juce::AudioProcessor>, PyAudioProcessor>(m, "AudioProcessor",
                                                                                              py::dynamic_attr())
        .def(py::init(
            []() { return createOnMessageThread<juce::AudioProcessor>([] { return new PyAudioProcessor(); }); }))
        ...
        .def("createEditor",
             [](juce::AudioProcessor &self) {
                 return createOnMessageThread<juce::AudioProcessorEditor>([&self] { return self.createEditor(); });
             })
