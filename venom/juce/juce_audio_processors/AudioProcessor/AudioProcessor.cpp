...
void init_AudioProcessor(py::module &m) {
    py::class_<juce::AudioProcessor, std::shared_ptr<juce::AudioProcessor>, PyAudioProcessor>(m, "AudioProcessor",
        ...
        .def(py::init([]() -> PyAudioProcessor * {
            ...
            auto createEditor = [](void *userData) -> void * { return new PyAudioProcessor(); };
            if (juce::MessageManager::getInstanceWithoutCreating()->isThisTheMessageThread()) {
                ...
                return static_cast<PyAudioProcessor *>(
                    juce::MessageManager::getInstanceWithoutCreating()->callFunctionOnMessageThread(createEditor,
                                                                                                    nullptr));
            }
        }))
